...
create table event ( id int not null auto_increment, `when` datetime(6) not null, authority varchar(255) not null, type varchar(255) not null, text text not null, primary key(id) );
...
create table election ( id int not null auto_increment, owner_id int not null, foreign key fk_owner(owner_id) references user(id), name varchar(255) not null unique, start datetime(6), end datetime(6), secret boolean not null default false, restricted_to_voter_list boolean not null default false, done_configuring datetime(6), template boolean not null default false, started boolean not null default false, finished boolean not null default false, can_change_candidates_after_done_configuring boolean not null default false, owner_can_delete_ballots boolean not null default false, auditor_can_delete_ballots boolean not null default false, primary key(id) );
